> ⚠️ **规划草稿，部分内容已过时**：本规划早期版本将部署写成 Cloudflare Pages、并计划接入 Supabase 云端留言板。当前项目已**清理全部后端依赖**，改为**纯静态 + GitHub Pages（`gh-pages` 分支）部署**，社区留言板为纯 localStorage。拼音搜索、输入校验加固等"待实施"项**多数已在代码中完成**。进度请以根目录 **`PROJECT_DOCUMENT.md`（权威版）** 为准。
>
> ---
