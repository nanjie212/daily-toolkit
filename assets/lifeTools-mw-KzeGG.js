import{Q as p}from"./index-CwuOcX83.js";import{t as M}from"./date-Bk9C5qmq.js";import"./vendor-CR0zKoQR.js";import"./lucide-C0ZlDHtg.js";import"./qrcode-DpynCwP9.js";async function F(c){try{const e=c.content,t=Number(c.size)||256,s=c.color||"#000000";if(!(e!=null&&e.trim()))return{success:!1,error:"请输入内容"};const o=await p.toDataURL(e,{width:t,margin:2,color:{dark:s,light:"#FFFFFF"},errorCorrectionLevel:"M"});return{success:!0,data:o,downloadUrl:o,filename:"qrcode.png"}}catch(e){return{success:!1,error:`生成失败: ${e.message}`}}}const N={m:1,km:1e3,cm:.01,in:.0254,ft:.3048,mi:1609.344},w={kg:1,g:.001,lb:.453592,oz:.0283495};function b(c,e,t){let s;switch(e){case"c":s=c;break;case"f":s=(c-32)*5/9;break;case"k":s=c-273.15;break;default:return c}switch(t){case"c":return s;case"f":return s*9/5+32;case"k":return s+273.15;default:return s}}async function L(c){try{const e=c.value;if(e===""||e===null||e===void 0)return{success:!1,error:"请输入数值"};const t=Number(e),s=c.category||"length",o=c.fromUnit,r=c.toUnit;if(isNaN(t))return{success:!1,error:"请输入有效数值"};let n;const a={m:"米",km:"千米",cm:"厘米",in:"英寸",ft:"英尺",mi:"英里",kg:"千克",g:"克",lb:"磅",oz:"盎司",c:"摄氏度",f:"华氏度",k:"开尔文"};if(s==="temperature")n=b(t,o,r);else{const i=s==="length"?N:w,l=i[o],f=i[r];if(!l||!f)return{success:!1,error:"不支持的单位"};n=t*l/f}return{success:!0,data:`${t} ${a[o]||o} = ${Number(n.toFixed(6))} ${a[r]||r}`}}catch(e){return{success:!1,error:`转换失败: ${e.message}`}}}async function C(c){try{const e=Number(c.principal)||1e6,t=Number(c.rate)||3.5,s=Number(c.years)||30,o=c.method||"equal-payment";if(e<=0)return{success:!1,error:"贷款金额必须大于0"};if(t<0||t>100)return{success:!1,error:"年利率应在0~100%之间"};if(s<=0||s>100)return{success:!1,error:"贷款年限应在1~100年之间"};const r=t/100/12,n=s*12;if(o==="equal-payment"){const a=e*r*Math.pow(1+r,n)/(Math.pow(1+r,n)-1),i=a*n,l=i-e;return{success:!0,data:`【等额本息】
...
还款总额: ${f.toFixed(2)}元
利息总额: ${u.toFixed(2)}元`}}}catch(e){return{success:!1,error:`计算失败: ${e.message}`}}}async function Y(c){try{const e=Number(c.salary)||0,t=Number(c.socialInsurance)||0,s=Number(c.threshold)||5e3,o=Number(c.deductions)||0;if(e<=0)return{success:!1,error:"请输入有效的税前月薪"};if(t>e)return{success:!1,error:"五险一金不应超过税前月薪"};const r=e-t-s-o;if(r<=0)return{success:!0,data:`【个税计算结果】
...
应缴个税: ${a.toFixed(2)}元
到手工资: ${f.toFixed(2)}元`}}catch(e){return{success:!1,error:`计算失败: ${e.message}`}}}async function z(c){try{const e=Number(c.weight),t=Number(c.height);if(!e||!t)return{success:!1,error:"请输入有效的体重和身高"};if(e<=0||e>500)return{success:!1,error:"体重应在0~500kg之间"};if(t<=0||t>300)return{success:!1,error:"身高应在0~300cm之间"};const s=t/100,o=e/(s*s);let r,n;return o<18.5?(r="偏瘦",n="建议适当增加营养摄入，均衡饮食，适度锻炼增强体质。"):o<24?(r="正常",n="体重在健康范围内，请继续保持良好的生活习惯。"):o<28?(r="偏胖",n="建议控制饮食，减少高热量食物摄入，增加有氧运动。"):(r="肥胖",n="建议咨询医生，制定科学的减重计划，注意饮食和运动。"),{success:!0,data:`【BMI计算结果】
体重: ${e}kg
身高: ${t}cm
...
相差: ${i}天 (${l}周${i%7}天)`}}else if(e==="add"){if(!o(t))return{success:!1,error:`"${t}" 不是有效日期`};const r=new Date(t),n=parseInt(s,10);if(isNaN(r.getTime()))return{success:!1,error:"日期格式无效，请使用 YYYY-MM-DD"};if(isNaN(n))return{success:!1,error:"请输入有效的天数"};r.setDate(r.getDate()+n);const a=M(r);return{success:!0,data:`【日期推算】
起始日期: ${t}
推算天数: ${n>0?"+":""}${n}天
...
已过去: ${Math.abs(i)}天`}}}catch(e){return{success:!1,error:`计算失败: ${e.message}`}}}async function v(c){try{const e=c.options,t=Number(c.count)||1;if(!(e!=null&&e.trim()))return{success:!1,error:"请输入选项"};const s=e.split(`
...
🎯 选中: ${o.join("、")}`}}catch(e){return{success:!1,error:`决策失败: ${e.message}`}}}async function j(c){try{const e=c.names,t=Number(c.count)||1,s=c.allowRepeat;if(!(e!=null&&e.trim()))return{success:!1,error:"请输入名单"};const o=e.split(`
...
${a.join(`
`)}`}}return{success:!1,error:'请先选择"开始计时"启动秒表'}}catch(e){return{success:!1,error:e.message}}}async function B(c){try{const e=c.targetDate,t=c.targetTime||"00:00",s=c.eventName||"目标";if(!e)return{success:!1,error:"请输入目标日期"};if(!/^\d{4}-\d{2}-\d{2}$/.test(e))return{success:!1,error:`"${e}" 不是有效日期`};const o=new Date(`${e}T${t}:00`);if(isNaN(o.getTime())||o.toISOString().slice(0,10)!==e)return{success:!1,error:`"${e}" 不是有效日期`};const r=new Date,n=o.getTime()-r.getTime();if(n<=0){const m=r.getTime()-o.getTime(),h=Math.floor(m/864e5),g=Math.floor(m%864e5/36e5);return{success:!0,data:`【${s}】
...
≈ ${u} 小时
≈ ${d} 分钟`}}catch(e){return{success:!1,error:e.message}}}export{z as bmiCalculator,B as countdown,I as dateCalculator,j as luckyDraw,C as mortgageCalculator,R as passwordGenerator,A as pomodoroTimer,F as qrcodeGenerator,v as randomDecision,q as sensitiveMask,P as simpleCalculator,V as stopwatch,Y as taxCalculator,U as timezoneConverter,L as unitConverter};
