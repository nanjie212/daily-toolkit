...
export async function fileCompress(input: Record<string, unknown>): Promise<ToolOutput> {
  try {
    const mode = (input.mode as string) || 'compress';
    const files = input.files as File | File[];

    if (!files) return { success: false, error: '请选择文件' };
    ...
    if (totalSize > MAX_SIZE) {
      return { success: false, error: `文件总大小(${(totalSize / 1024 / 1024).toFixed(1)}MB)超过限制(100MB)，请选择更小的文件` };
    }

    if (mode === 'decompress') {
      const zipFile = fileArray[0];
      if (!zipFile.name.endsWith('.zip')) {
        return { success: false, error: '解压模式仅支持ZIP文件' };
      }
      ...
      for (const name of fileNames) {
        ...
        if (content) newZip.file(name, content);
      }
      ...
      return {
        ...
        data: {
          操作: '解压ZIP',
          文件数量: fileNames.length,
          文件列表: fileNames.slice(0, 20).join('\n'),
          提示: fileNames.length > 20 ? `仅显示前20个文件，共${fileNames.length}个` : '',
        },
        downloadUrl,
    ...
    return {
      ...
      data: {
        ...
        原始大小: `${(totalOriginalSize / 1024).toFixed(1)} KB`,
        压缩后大小: `${(blob.size / 1024).toFixed(1)} KB`,
        压缩率: `${((1 - blob.size / totalOriginalSize) * 100).toFixed(1)}%`,
      },
      downloadUrl,
