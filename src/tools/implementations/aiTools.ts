...
export async function speechToText(input: Record<string, unknown>): Promise<ToolOutput> {
  try {
    ...
    const SpeechRecognition = (win['SpeechRecognition'] || win['webkitSpeechRecognition']) as (() => void) | undefined;
    if (!SpeechRecognition) {
      return {
        success: false,
        error: '您的浏览器不支持语音识别功能。请使用Chrome浏览器，并确保已授权麦克风权限。',
      };
    }

    return {
      success: true,
      data: `【语音转文字使用说明】\n\n本工具使用浏览器内置的 Web Speech API。\n\n使用方法:\n1. 请使用 Chrome 浏览器打开本工具\n2. 点击浏览器的麦克风图标授权\n3. 对着麦克风说话即可实时识别\n4. 识别语言: ${language === 'zh-CN' ? '中文' : language === 'en-US' ? '英文' : language === 'ja-JP' ? '日文' : '韩文'}\n\n提示: 语音识别需要联网，且仅 Chrome 浏览器支持此功能。\n如需使用，请在浏览器中直接按 F12 打开控制台，输入以下代码测试:\n\nconst r = new (window.SpeechRecognition || window.webkitSpeechRecognition)();\nr.lang = '${language}';\nr.onresult = e => console.log(e.results[0][0].transcript);\nr.start();`,
    };
  } catch (e) {
